...
## Tieba Data Collection (Removed)

The Baidu Tieba (贴吧) cross-platform scraper was **removed** in June 2026 (49 files deleted: JS services, Python CLI modules, corpus handlers, and scraper planners). The rationale for cross-platform validation remains valid (see historical rationale below), but the implementation was retired to narrow the project's scope to Bilibili-only analysis.

68 Python contract tests are preserved with `@unittest.skip("Tieba scraper removed")` as historical markers. The task config at `.claude/tasks/tieba_scrape.json` is defunct. To revive Tieba support, revert commit history — all removed code is preserved in git.

---

## Current Dictionary Status

Dictionary coverage stats are auto-generated by `npm run stats:update` and displayed in the [Data Growth](#data-growth--数据增长) section above. See `server/data/keywordCoverageAudit.json` for the full per-term audit, and run `npm run dictionary:coverage` to refresh it.
...
百度贴吧跨平台抓取器已于 2026 年 6 月**移除**（删除 49 个文件：JS 服务、Python CLI 模块、语料库处理器和抓取器规划器）。跨平台验证的理论依据仍然成立（见下方历史理由），但实现已被移除，以将项目范围缩小至仅限 B 站分析。

68 个 Python 合约测试保留并标记为 `@unittest.skip("Tieba scraper removed")` 作为历史标记。`.claude/tasks/tieba_scrape.json` 任务配置已失效。如需恢复贴吧支持，可在 git 历史中恢复 —— 所有已删除代码均保留在版本历史中。

---

## 当前词典状态

词典覆盖统计由 `npm run stats:update` 自动生成，显示在上方[数据增长](#data-growth--数据增长)区域。完整的逐项审计见 `server/data/keywordCoverageAudit.json`，运行 `npm run dictionary:coverage` 可刷新。
...
| `tieba-scrape` | ~~tieba-keyword-scrape~~ | — | REMOVED: Tieba scraper deleted (49 files). Task config defunct. / 已移除：贴吧抓取器已删除 |
