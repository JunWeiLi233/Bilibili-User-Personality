...
from python_backend.scrapers.batch_uid_range import RangeScraperLauncherContractComparator as RangeScraperLauncherPayloadComparator, RangeScraperLauncherPlanner, RangeScraperLauncherSummary
...
class RangeScraperLauncherPlanRunner:
    ...
    def run(self) -> dict[str, Any]:
        return RangeScraperLauncherPlanner().build_plan(data_dir=self.data_dir, script=self.script, launch_delay_seconds=self.launch_delay_seconds)


class RangeScraperLauncherContractComparator:
    """Compare Python range scraper launch plans against saved JS-compatible JSON."""

    def __init__(self, data_dir: str | Path, js_report_path: str | Path):
        self.data_dir = Path(data_dir)
        self.js_report_path = Path(js_report_path)
        self.summary = RangeScraperLauncherSummary()
        self.comparator = RangeScraperLauncherPayloadComparator(self.summary)
