...
class NearTargetResolvePlanRunner:
    ...
    def run(self) -> dict[str, Any]:
        ...
        return {
            ...
            "videosPlanned": sum(len(plan["bvids"]) for plan in plans),
            "plans": plans,
            "skipped": skipped,
            "summary": {
                "candidateCount": len(candidates),
    ...
    def _evidence_needed(self, entry: dict[str, Any]) -> int:
        audit = CoverageAuditBuilder(target_evidence=self.target_evidence, require_source_backed_evidence=True, require_comment_backed_evidence=False)
