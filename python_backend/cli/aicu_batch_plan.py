...
from python_backend.scrapers.aicu import AicuBatchPlanContractComparator as AicuBatchPlanPayloadComparator, AicuBatchPlanSummary, AicuBatchPlanner
...
class AicuBatchPlanContractComparator:
    """Compare Python AICU batch plans against saved JS-compatible JSON."""

    def __init__(self, payload_path: str | Path, js_report_path: str | Path):
        self.payload_path = Path(payload_path)
        self.js_report_path = Path(js_report_path)
        self.summary = AicuBatchPlanSummary()
        self.comparator = AicuBatchPlanPayloadComparator(self.summary)
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.compare_js_report:
        result = AicuBatchPlanContractComparator(args.payload, args.compare_js_report).compare()
    else:
        result = AicuBatchPlanRunner(args.payload).run()
