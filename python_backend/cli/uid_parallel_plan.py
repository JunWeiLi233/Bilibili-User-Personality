...
from python_backend.scrapers.uid_parallel import UidParallelAnalyzerPlanner, UidParallelPlanContractComparator as UidParallelPlanPayloadComparator, UidParallelPlanSummary
...
class UidParallelPlanRunner:
    ...
    def _read_payload(self) -> dict[str, Any]:
        ...
        if not isinstance(payload, dict):
            raise ValueError("UID parallel plan payload must be a JSON object.")
        return payload


class UidParallelPlanContractComparator:
    """Compare Python UID parallel dry-run plans against saved JS-compatible JSON."""

    def __init__(self, payload_path: str | Path, js_report_path: str | Path):
        self.payload_path = Path(payload_path)
        self.js_report_path = Path(js_report_path)
        self.summary = UidParallelPlanSummary()
        self.comparator = UidParallelPlanPayloadComparator(self.summary)
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.compare_js_report:
        result = UidParallelPlanContractComparator(args.payload, args.compare_js_report).compare()
    else:
        result = UidParallelPlanRunner(args.payload).run()
