...
class UidPipelinePlanRequest:
    """Scraper-layer request for UID pipeline plan JSON contract commands."""
    ...
    def run(self) -> dict[str, Any]:
        if self.compare_js_report_path:
            return UidPipelinePlanPayloadContractComparator(self.payload_path, self.compare_js_report_path).compare()
        return UidPipelinePlanRunner(self.payload_path).run()


class UidPipelinePlanCommandRequest:
    """Argv-backed scraper-layer request for UID pipeline plan contracts."""
    ...
    def run(self) -> dict[str, Any]:
        ...
        return UidPipelinePlanRequest(args.payload, compare_js_report_path=args.compare_js_report or None).run()
...
class UidPipelineLauncherRequest:
    """Scraper-layer request for UID pipeline launcher JSON contract commands."""
    ...
    def run(self) -> dict[str, Any]:
        if self.compare_js_report_path:
            return UidPipelineLauncherPayloadContractComparator(
                self.data_dir,
...
class UidPipelineLauncherCommandRequest:
    """Argv-backed scraper-layer request for UID pipeline launcher contracts."""
