...
from python_backend.analyzers.deepseek_cli import DeepSeekAnalyzeCliPayloadPlanContractComparator, DeepSeekAnalyzeCliPlanContractComparator as DeepSeekAnalyzeCliPlanPayloadComparator, DeepSeekAnalyzeCliPlanRunner as DeepSeekAnalyzeCliPayloadPlanRunner, DeepSeekAnalyzeCliPlanner, DeepSeekAnalyzeCliPlanSummary
...
from python_backend.corpus.direct_probe import DirectProbeCorpusBuilder, DirectProbeCorpusContractComparator as DirectProbeCorpusPayloadComparator, DirectProbeCorpusJsonPayloadContractComparator, DirectProbeCorpusPayloadContractComparator, DirectProbeCorpusRequest, DirectProbeCorpusRunner as DirectProbePayloadCorpusRunner, DirectProbeCorpusSummary, DirectProbePlanContractComparator as DirectProbePlanPayloadComparator, DirectProbePlanPayloadContractComparator, DirectProbePlanRunner as DirectProbePayloadPlanRunner, DirectProbePlanSummary
...
class CorpusContractTests(unittest.TestCase):
    ...
    def test_writer_constructor_defaults_invalid_max_shard_bytes_like_js(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            output = root / "out.json"
            CorpusShardWriter(output, max_shard_bytes="not-a-number").write(
                comments=[{"message": "constructor comment"}],
                runs=[],
                manifest={"version": 1},
            )
    ...
    def test_random_verification_request_runs_path_contract_with_extra_corpus(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            corpus_path = root / "corpus.json"
            tieba_path = root / "tiebaKeywordCorpus.json"
            ...
            result = RandomVerificationRequest(
    ...
    def test_random_verification_cli_compare_keeps_js_sample_defaults(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            corpus_path = root / "corpus.json"
            dictionary_path = root / "dictionary.json"
            js_report_path = root / "js-random.json"
            corpus_path.write_text(
                json.dumps({"comments": [{"message": "doge one"}, {"message": "doge two"}, {"message": "doge three"}], "runs": []}),
                encoding="utf-8",
            )
    ...
    def test_aicu_scrape_planner_defaults_invalid_payload_numeric_fields(self):
        result = AicuScrapePlanner.build_plan_from_payload(
            {
                "argv": ["--uid=42"],
                "maxPages": "bad",
                "pageSize": "also-bad",
                "delayBetweenUidsMs": "invalid",
            }
        )

        self.assertEqual(result["summary"], {"uids": 1, "commentPagesPerUid": 10, "danmakuPagesPerUid": 10, "delayBetweenUidsMs": 15000})
        self.assertEqual(result["requests"][0]["commentsUrl"], "https://api.aicu.cc/api/v3/search/getreply?uid=42&pn=1&ps=20&mode=0&keyword=")
    ...
    def test_aicu_batch_planner_matches_js_resume_waf_and_request_contract(self):
        ...
        progress = {"lastUid": 100002, "completed": 4, "errors": [{"uid": "100001", "error": "HTTP 429"}]}
        ...
        self.assertEqual(result["progress"], {"lastUid": 100002, "completed": 4, "errors": 1})
        self.assertEqual(result["database"], {"users": 2, "existingInEffectiveRange": 1})
        self.assertEqual(result["limits"], {"maxPages": 3, "pageSize": 20, "saveEveryAttempts": 5})
        self.assertEqual(result["pacing"], {"delayBetweenPagesMs": 10000, "delayBetweenUidsMs": 20000, "delayAfterWafMs": 120000})
        self.assertEqual(result["retry"], {"maxRetries": 3, "wafStatuses": [429, 468, 1015], "headers": {"accept": "application/json", "referer": "https://www.aicu.cc/", "hasUserAgent": True}})
        self.assertEqual(
    ...
    def test_aicu_batch_plan_comparator_uses_summary_contract_keys(self):
        self.assertFalse(hasattr(AicuBatchPlanContractComparator, "RESULT_KEYS"))
        self.assertEqual(
    ...
    def test_aicu_batch_plan_runner_and_comparator_read_json_contracts(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            js_report_path.write_text(json.dumps({"range": {"effectiveStart": 1}, "limits": {"maxPages": 10}}), encoding="utf-8")

            result = AicuBatchPlanRunner(payload_path).run()
            comparison = AicuBatchPlanContractComparator(payload_path, js_report_path).compare()
        ...
        self.assertEqual(
            comparison["mismatches"],
            [
                {"key": "range", "python": {"requestedStart": 1, "effectiveStart": 2, "end": 3, "total": 2}, "js": {"effectiveStart": 1}},
    ...
    def test_aicu_batch_plan_runner_defaults_non_object_payload_root(self):
        with tempfile.TemporaryDirectory() as tmp:
            payload_path = Path(tmp) / "aicu-batch-plan.json"
            payload_path.write_text(json.dumps(["bad", "payload"]), encoding="utf-8")
        ...
        self.assertEqual(result["database"], {"users": 0, "existingInEffectiveRange": 0})

    def test_aicu_batch_plan_payload_runner_lives_with_scraper_logic(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            payload_path = root / "aicu-batch-plan.json"
            js_report_path = root / "js-aicu-batch-plan.json"
                ...
                json.dumps(
                    {
                        ...
                        "progress": {"lastUid": 20, "completed": "2", "errors": [{"uid": "20"}]},
                        "database": {"users": {"21": {}, "30": {}}},
                    }
                ),
                encoding="utf-8",
            )
            js_report_path.write_text(json.dumps({"range": {"effectiveStart": 20}, "progress": {"completed": 0}}), encoding="utf-8")

            result = AicuBatchPayloadPlanRunner(payload_path).run()
    ...
    def test_aicu_browser_batch_plan_payload_comparator_owns_result_key_mismatch_contract(self):
        result = AicuBrowserBatchPlanPayloadComparator().compare(
            {
                "ok": True,
                "range": {"effectiveStart": 21},
                "browser": {"maxPages": 3},
                "pacing": {"delayBetweenUidsMs": 5000},
                "extra": "ignored",
            },
        ...
        self.assertEqual(
            result["mismatches"],
            [
                {"key": "range", "python": {"effectiveStart": 21}, "js": {"effectiveStart": 20}},
                {"key": "browser", "python": {"maxPages": 3}, "js": {"maxPages": 1}},
            ],
        )
        self.assertEqual(
            result["python"],
            {"range": {"effectiveStart": 21}, "browser": {"maxPages": 3}, "pacing": {"delayBetweenUidsMs": 5000}},
        )
        self.assertEqual(
            result["js"],
            {"range": {"effectiveStart": 20}, "browser": {"maxPages": 1}, "pacing": {"delayBetweenUidsMs": 5000}},
        )
    ...
    def test_video_link_direct_plan_request_owns_cli_dispatch(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            run_result = VideoLinkDirectPlanRequest(payload_path).run()
            compare_result = VideoLinkDirectPlanRequest(payload_path, compare_js_report_path=js_report_path).run()

        self.assertTrue(run_result["ok"])
        self.assertEqual(run_result["mode"], "uid")
    ...
    def test_aicu_scrape_plan_request_owns_cli_dispatch(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            payload_path = root / "aicu-plan.json"
            js_report_path = root / "js-aicu-plan.json"
            ...
            run_result = AicuScrapePlanRequest(payload_path).run()
            compare_result = AicuScrapePlanRequest(payload_path, compare_js_report_path=js_report_path).run()
    ...
    def test_scraper_adapter_builds_metadata_request_from_json_payload_contract(self):
        ...
        result = scraper.build_metadata_request_from_payload({"keyword": "弹幕", "pageSize": 0, "source": "bilibili"})
    ...
    def test_keyword_evidence_request_compares_payload_contract(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            js_report_path.write_text(json.dumps({"ok": True, "mode": "entries", "count": 0, "entries": []}), encoding="utf-8")

            result = KeywordEvidenceRequest(payload_path=payload_path, compare_js_report_path=js_report_path).run()

        self.assertFalse(result["ok"])
        self.assertEqual([item["key"] for item in result["mismatches"]], ["count", "entries"])
    ...
    def test_semantic_matcher_request_owns_cli_dispatch(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            payload_path = root / "semantic-payload.json"
            js_report_path = root / "js-semantic.json"
            ...
            run_result = SemanticMatcherRequest(payload_path).run()
            compare_result = SemanticMatcherRequest(payload_path, compare_js_report_path=js_report_path).run()

        self.assertTrue(run_result["ok"])
        self.assertEqual(run_result["matches"], [{"term": "term-a", "chunk": "alpha chunk", "score": 1.0}])
        self.assertFalse(compare_result["ok"])
    ...
    def test_deepseek_analyze_cli_payload_plan_runner_lives_with_analyzer_cli_logic(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            payload_path = root / "deepseek-cli.json"
            js_report_path = root / "js-deepseek-cli.json"
            ...
            result = DeepSeekAnalyzeCliPayloadPlanRunner(payload_path).run()
            comparison = DeepSeekAnalyzeCliPayloadPlanContractComparator(payload_path, js_report_path).compare()
    ...
    def test_deepseek_analysis_plan_runner_defaults_non_object_payload_root(self):
        with tempfile.TemporaryDirectory() as tmp:
            payload_path = Path(tmp) / "payload.json"
            payload_path.write_text(json.dumps(["bad", "payload"]), encoding="utf-8")
    ...
    def test_deepseek_analysis_plan_cli_runner_accepts_argv_payload_contract(self):
        sentence = "\u8fd9\u53e5\u662f\u5728\u53cd\u8bbd\u5427[doge]"
        with tempfile.TemporaryDirectory() as tmp:
            payload_path = Path(tmp) / "payload.json"
            payload_path.write_text(
            ...
            result = deepseek_analysis_plan_cli.DeepSeekAnalysisPlanCliRunner(["--payload", str(payload_path)]).run()
    ...
    def test_deepseek_analysis_plan_request_compares_js_contract(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            payload_path = root / "payload.json"
            js_plan_path = root / "js-plan.json"
            ...
            result = DeepSeekAnalysisPlanRequest(payload_path=payload_path, compare_js_plan_path=js_plan_path).run()
    ...
    def test_deepseek_analysis_payload_validate_runner_lives_with_analyzer_validator(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            payload_path = root / "payload.json"
            analysis_path = root / "analysis.json"
            ...
            result = DeepSeekAnalysisPayloadValidateRunner(payload_path, analysis_path).run()
            comparison = DeepSeekAnalysisPayloadValidateContractComparator(payload_path, analysis_path, js_report_path).compare()

        self.assertFalse(result["ok"])
        self.assertEqual(result["unsupportedQuotes"], [{"path": "sentenceAnalyses[0].quote", "quote": "\u5e7b\u89c9\u5f15\u7528"}])
        self.assertFalse(comparison["ok"])
        self.assertEqual([item["key"] for item in comparison["mismatches"]], ["ok", "summary"])

    def test_deepseek_analysis_validate_request_compares_js_contract(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            payload_path = root / "payload.json"
            analysis_path = root / "analysis.json"
            ...
            result = DeepSeekAnalysisValidateRequest(
                payload_path=payload_path,
    ...
    def test_dictionary_loader_normalizes_non_object_root_like_js(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            (root / "dict.json").write_text(json.dumps("bad dictionary root"), encoding="utf-8")
    ...
    def test_huggingface_import_cli_accepts_argv_plan_payload_contract(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            payload_path = root / "hf-plan.json"
            payload_path.write_text(
            ...
            output = io.StringIO()
            with contextlib.redirect_stdout(output):
                exit_code = huggingface_corpus_cli.main(["--plan-payload", str(payload_path)])
    ...
    def test_huggingface_import_plan_payload_comparator_lives_with_corpus_logic(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            js_report_path.write_text(json.dumps({"requestTimeoutMs": 500, "summary": {"sources": 2}}), encoding="utf-8")

            comparison = HuggingFaceCorpusImportPlanPayloadComparator(payload_path, js_report_path).compare()

        self.assertFalse(comparison["ok"])
        self.assertEqual(
            comparison["mismatches"],
            [
                {"key": "requestTimeoutMs", "python": 1000, "js": 500},
    ...
    def test_local_corpus_flatten_request_compares_js_contract(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            input_path = root / "local.json"
            js_report_path = root / "js-flatten.json"
            ...
            js_report_path.write_text(json.dumps({"count": 0, "comments": []}), encoding="utf-8")

            result = LocalCorpusFlattenRequest(
    ...
    def test_local_corpus_evidence_finder_ignores_non_array_existing_evidence_fields(self):
        dictionary = {
            "entries": [
                {
                    "term": "\u72d7\u5934",
    ...
    def test_local_corpus_evidence_payload_runner_lives_with_corpus_logic(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            result = LocalCorpusEvidencePayloadRunner(
                dictionary_path,
                comments_path,
                target_evidence=3,
            ...
            comparison = LocalCorpusEvidencePayloadContractComparator(
                dictionary_path,
                comments_path,
                js_report_path,
                target_evidence=3,
    ...
    def test_local_corpus_evidence_request_compares_json_payload_contract(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            result = LocalCorpusEvidenceRequest(
                payload_path=payload_path,
                compare_js_report_path=js_report_path,
            ).run()
    ...
    def test_local_corpus_mine_plan_runner_defaults_non_object_payload_root(self):
        with tempfile.TemporaryDirectory() as tmp:
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            payload_path.write_text(
                json.dumps(
                    {
                        "existing": {"version": 1, "runs": [], "comments": []},
                        "run": {"results": [{"comments": [{"message": "\u5355\u6587\u4ef6\u8d34\u5427\u5bf9\u6bd4", "sourceUrl": "https://tieba.baidu.com/p/5"}]}]},
                        "generatedAt": "2026-06-17T05:00:00.000Z",
                    }
                ),
                encoding="utf-8",
            ...
            output = io.StringIO()
            with contextlib.redirect_stdout(output):
                exit_code = tieba_corpus_cli.main(["--payload", str(payload_path), "--compare-js-report", str(js_report_path)])
    ...
    def test_tieba_corpus_payload_comparator_lives_with_corpus_logic(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            result = TiebaCorpusUpdatePayloadRunner(existing_path, run_path, generated_at="2026-06-17T03:00:00.000Z").run()
        ...
        self.assertTrue(result["changed"])
        self.assertEqual(result["newComments"], [{"message": "payload tieba comment", "sourceUrl": "https://tieba.baidu.com/p/3"}])
        self.assertFalse(comparison["ok"])
        self.assertEqual(
            comparison["mismatches"],
            [
    ...
    def test_direct_probe_corpus_cli_runner_reads_payload_contract(self):
        with tempfile.TemporaryDirectory() as tmp:
            payload_path = Path(tmp) / "direct-probe-corpus.json"
            payload_path.write_text(
                json.dumps(
                    {
                        "existing": {"version": 1, "comments": [], "runs": []},
                        "comments": [{"message": "\u76f4\u63a5\u8ddf\u8fdb\u8bc4\u8bba", "source": "fresh", "uid": "84"}],
                        "run": {"at": "2026-06-18T02:00:00.000Z", "query": "\u76f4\u63a5\u8ddf\u8fdb"},
                    }
                ),
                encoding="utf-8",
            )

            result = direct_probe_corpus_cli.DirectProbeCorpusCliRunner(["--payload", str(payload_path)]).run()

        self.assertTrue(result["ok"])
        self.assertEqual(result["corpus"]["comments"][0]["message"], "\u76f4\u63a5\u8ddf\u8fdb\u8bc4\u8bba")
        self.assertEqual(result["corpus"]["runs"][0]["commentsAdded"], 1)

    def test_direct_probe_corpus_cli_compares_payload_contract(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            payload_path = root / "direct-probe-corpus.json"
            js_report_path = root / "js-report.json"
            payload_path.write_text(
                json.dumps(
                    {
                        "existing": {"version": 1, "comments": [], "runs": []},
                        "comments": [{"message": "\u76f4\u63a5\u5bf9\u6bd4\u8bc4\u8bba", "source": "fresh", "uid": "42"}],
                        "run": {"at": "2026-06-18T02:00:00.000Z", "query": "\u76f4\u63a5\u5bf9\u6bd4"},
                    }
                ),
                encoding="utf-8",
            )
            ...
            output = io.StringIO()
            with contextlib.redirect_stdout(output):
                exit_code = direct_probe_corpus_cli.main(["--payload", str(payload_path), "--compare-js-report", str(js_report_path)])
    ...
    def test_direct_probe_corpus_request_compares_payload_contract(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            payload_path = root / "direct-probe-corpus.json"
            js_report_path = root / "js-report.json"
            payload_path.write_text(
                json.dumps(
                    {
                        "existing": {"version": 1, "comments": [], "runs": []},
                        "comments": [{"message": "\u76f4\u63a5\u8bf7\u6c42\u8bc4\u8bba", "source": "fresh", "uid": "42"}],
                        "run": {"at": "2026-06-18T02:00:00.000Z", "query": "\u76f4\u63a5\u8bf7\u6c42"},
                    }
                ),
                encoding="utf-8",
            )
    ...
    def test_direct_probe_corpus_payload_runner_lives_with_corpus_logic(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            result = DirectProbePayloadCorpusRunner(existing_path, comments_path, run_path).run()
            comparison = DirectProbeCorpusPayloadContractComparator(existing_path, comments_path, run_path, js_report_path).compare()

        self.assertEqual(result["corpus"]["comments"][0]["message"], "\u65b0\u8bc4\u8bba")
        self.assertFalse(comparison["ok"])
        self.assertEqual([item["key"] for item in comparison["mismatches"]], ["commentMessages", "runQueries", "runCommentsAdded"])
    ...
    def test_history_tag_corpus_shard_writer_round_trips_through_loader(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            output_path = root / "history-tags.json"
            tags = [{"name": f"\u5386\u53f2{i}", "source": "seed"} for i in range(3)]
            videos = [
                {"bvid": f"BVhistory{i}", "title": f"\u5386\u53f2\u89c6\u9891{i}", "tags": ["\u5386\u53f2"], "replyCount": i}
                for i in range(3)
            ]
            runs = [{"at": f"run-{i}"} for i in range(2)]

            HistoryTagCorpusShardWriter(output_path, max_shard_bytes=1024).write(
                tags=tags,
                videos=videos,
                runs=runs,
        ...
        self.assertEqual(loaded["storage"], "split")
        self.assertEqual(loaded["tagCount"], 3)
        self.assertEqual(loaded["videoCount"], 3)
        self.assertEqual(loaded["runCount"], 2)
    ...
    def test_history_tag_scrape_plan_runner_defaults_non_object_payload_root_without_strict_guard(self):
        with tempfile.TemporaryDirectory() as tmp:
            payload_path = Path(tmp) / "payload.json"
            payload_path.write_text(json.dumps(["not", "an", "object"]), encoding="utf-8")
    ...
    def test_tieba_html_parse_payload_runner_lives_with_scraper_logic(self):
        ...
        self.assertTrue(result["ok"])
        self.assertEqual(result["threads"][0]["title"], "\u8d34\u5427\u6897\u8ba8\u8bba")
        self.assertTrue(comparison["ok"])
    ...
    def test_bilibili_parse_request_owns_cli_dispatch(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            run_result = BilibiliParseRequest(payload_path).run()
            compare_result = BilibiliParseRequest(payload_path, compare_js_report_path=js_report_path).run()
    ...
    def test_bilibili_probe_plan_payload_runner_lives_with_scraper_logic(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            comparison = BilibiliProbePlanPayloadContractComparator(payload_path, js_report_path).compare()
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            payload_path.write_text(
                json.dumps(
                    {
                        "dictionary": {"entries": [{"term": "\u7f51\u76d8\u89c1", "family": "evasion"}]},
                        "comments": [{"message": "\u7f51\u76d8\u89c1"}, {"message": "\u666e\u901a\u8bc4\u8bba"}, {"message": "ascii only"}],
                        "sampleSize": 2,
                    }
                ),
                encoding="utf-8",
    ...
    def test_comment_coverage_cli_accepts_payload_contract(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            output = io.StringIO()
            with contextlib.redirect_stdout(output):
                exit_code = comment_coverage_cli.main(["--payload", str(payload_path)])
    ...
    def test_comment_coverage_cli_compares_payload_contract(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            output = io.StringIO()
            with contextlib.redirect_stdout(output):
                exit_code = comment_coverage_cli.main(["--payload", str(payload_path), "--compare-js-report", str(js_report_path)])
    ...
    def test_comment_coverage_request_compares_payload_contract(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            result = CommentCoverageRequest(payload_path=payload_path, compare_js_report_path=js_report_path).run()
    ...
    def test_coverage_progress_request_compares_payload_contract(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            payload_path = root / "payload.json"
            js_report_path = root / "js-progress.json"
            ...
            js_report_path.write_text(
                json.dumps({"delta": {"evidenceDeficitReduced": 0}, "hasGateProgress": False}),
                encoding="utf-8",
            )

            result = CoverageProgressRequest(payload_path=payload_path, compare_js_report_path=js_report_path).run()

        self.assertFalse(result["ok"])
        self.assertEqual([item["key"] for item in result["mismatches"]], ["delta", "hasGateProgress"])
    ...
    def test_uid_pipeline_merge_payload_comparator_owns_result_key_mismatch_contract(self):
        result = UidPipelineMergePayloadComparator().compare(
            {
                "lastUpdated": "",
                "totalProcessed": 1,
                "stats": {"success": 1, "errors": 0},
        ...
        self.assertEqual(
            ...
            [
                ...
                {"key": "stats", "python": {"success": 1, "errors": 0}, "js": {"success": 0, "errors": 0}},
            ],
        )
        self.assertEqual(
            result["python"],
            {"totalProcessed": 1, "stats": {"success": 1, "errors": 0}, "summary": {"totalProcessed": 1}},
        )
        self.assertEqual(result["js"], {"totalProcessed": 0, "stats": {"success": 0, "errors": 0}})
    ...
    def test_uid_pipeline_launcher_payload_runner_lives_with_scraper_logic(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            result = UidPipelineLauncherPayloadPlanRunner(
                data_dir,
                total_start=2,
                total_end=4,
                workers=2,
                write_state=True,
                now=lambda: "2026-06-20T00:00:00.000Z",
            ).run()
            js_report_path.write_text(json.dumps(result["state"]), encoding="utf-8")
            ...
            state_exists = (data_dir / "uid-pipeline-launcher.json").exists()

        self.assertEqual(result["statePath"], str(data_dir / "uid-pipeline-launcher.json"))
                ...
                json.dumps(
                    {
                        ...
                        "workers": [{"start": 7, "end": 8, "progressFile": "uid-pipeline-7-8.json", "processed": 1, "total": 2, "complete": False}],
                        "summary": {"workers": 1, "completedWorkers": 0, "totalProcessed": 1, "totalExpected": 2, "completionRatio": 0.5},
                        "stats": {"success": 1, "noComments": 0, "noVideos": 0, "noUser": 0, "trainError": 0, "blocked": 0, "errors": 0},
                    }
                ),
                encoding="utf-8",
            )
    ...
    def test_uid_pipeline_state_comparator_uses_summary_contract_keys(self):
        self.assertFalse(hasattr(UidPipelineStateContractComparator, "RESULT_KEYS"))
        self.assertEqual(
            UidPipelineStateContractComparator(Path("data"), Path("js.json")).summary.RESULT_KEYS,
            UidPipelineStateSummary.RESULT_KEYS,
        )

    def test_uid_pipeline_state_payload_comparator_owns_result_key_mismatch_contract(self):
        result = UidPipelineStatePayloadComparator().compare(
            {
                "startedAt": "2026-06-19T00:00:00.000Z",
                "workers": [{"start": 1, "end": 1, "processed": 1}],
                "summary": {"totalProcessed": 1, "completionRatio": 1.0},
                "stats": {"success": 1},
            },
            {
        ...
        self.assertEqual(
            result["mismatches"],
            [
                {"key": "summary", "python": {"totalProcessed": 1, "completionRatio": 1.0}, "js": {"totalProcessed": 0}},
        ...
        self.assertEqual(result["js"], {"summary": {"totalProcessed": 0}, "stats": {"success": 0}})
    ...
    def test_uid_pipeline_progress_payload_runner_lives_with_scraper_logic(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            js_report_path.write_text(
                json.dumps(
                    {
                        "range": {"start": 21, "end": 23, "total": 3},
                        "progress": {"processed": 2, "remaining": 1, "completionRatio": 0.6667},
                        "stats": {"success": 1, "noComments": 0, "noVideos": 0, "noUser": 0, "trainError": 0, "blocked": 1, "errors": 0},
                        "statusCounts": {"success": 1, "blocked": 1},
    ...
    def test_uid_pipeline_progress_comparator_uses_summary_contract_keys(self):
        self.assertFalse(hasattr(UidPipelineProgressContractComparator, "RESULT_KEYS"))
        self.assertEqual(
    ...
    def test_uid_fast_pipeline_payload_runner_lives_with_scraper_logic(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            payload_path.write_text(
                json.dumps(
                    {
                        "argv": ["--start=40", "--end=42"],
                        "progress": {"processed": {"40": "success", "41": "blocked"}, "stats": {"success": 1, "blocked": 1}},
                        "database": {"users": {"40": {}, "100": {}}},
                    }
                ),
                encoding="utf-8",
            )
            result = UidFastPipelinePayloadPlanRunner(payload_path).run()
            js_report_path.write_text(json.dumps(result), encoding="utf-8")

            comparison = UidFastPipelinePlanPayloadContractComparator(payload_path, js_report_path).compare()

        self.assertEqual(result["range"], {"start": 40, "end": 42, "total": 3})
        self.assertEqual(result["userDb"], {"users": 2, "usersInRange": 1})
        self.assertTrue(comparison["ok"])
        self.assertEqual(comparison["mismatches"], [])
    ...
    def test_uid_parallel_progress_summary_extracts_comparator_contract(self):
        summary = UidParallelProgressSummary().summarize(
            {
                "ok": True,
                "worker": {"id": 1},
    ...
    def test_uid_parallel_progress_payload_runner_lives_with_scraper_logic(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            data_dir = root / "server" / "data"
            data_dir.mkdir(parents=True)
            ...
            js_report_path.write_text(
                json.dumps(
                    {
                        "worker": {"id": 1, "totalWorkers": 2, "assigned": 1},
                        "progress": {"processed": 1, "remaining": 0, "completionRatio": 1.0},
    ...
    def test_uid_parallel_progress_payload_comparator_owns_result_key_mismatch_contract(self):
        result = UidParallelProgressPayloadComparator().compare(
            {
                "worker": {"id": 0, "totalWorkers": 2, "assigned": 2},
                "progress": {"processed": 1, "remaining": 1, "completionRatio": 0.5},
                "stats": {"success": 1},
                "lastUpdated": "ignored",
            },
    ...
    def test_uid_parallel_plan_payload_runner_lives_with_scraper_logic(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            payload_path.write_text(
                json.dumps(
                    {
                        "argv": ["--worker=1", "--workers=2"],
                        "comments": {"100": [{"message": "a"}], "101": [{"message": "b"}], "102": [{"message": "c"}]},
                        "progress": {"processed": {"101": "success"}, "stats": {"success": 1}},
                        "database": {"users": {"101": {}, "102": {}}},
                    }
                ),
                encoding="utf-8",
            )
            js_report_path.write_text(
                json.dumps(
                    {
                        "worker": {"id": 1, "totalWorkers": 2, "assigned": 1},
                        "assignment": {"assignedUids": ["101"], "alreadyProcessed": 1, "pending": 0, "trainable": 0, "skippableNoText": 0},
                        "userDb": {"users": 2, "assignedUsersInDb": 1},
                    }
                ),
                encoding="utf-8",
        ...
        self.assertTrue(result["ok"])
        self.assertEqual(result["worker"], {"id": 1, "totalWorkers": 2, "assigned": 1})
        self.assertEqual(result["assignment"]["assignedUids"], ["101"])
        self.assertTrue(comparison["ok"])
        self.assertEqual(comparison["js"]["worker"], result["worker"])
    ...
    def test_batch_scraper_launcher_plan_runner_matches_js_range_contract(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            result = BatchScraperLauncherPlanRunner(data_dir).run()
    ...
    def test_batch_scraper_launcher_comparator_reports_worker_mismatches(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            result = BatchScraperLauncherContractComparator(data_dir, js_report_path).compare()
    ...
    def test_batch_scraper_launcher_payload_comparator_owns_worker_mismatch_contract(self):
        result = BatchScraperLauncherPayloadComparator().compare(
            {
                "workers": [{"start": 1, "end": 20000, "progressFile": "batch-uid-progress-1-20000.json"}],
                "summary": {"workers": 1, "totalUids": 20000},
            },
            {
                "workers": [{"start": 1, "end": 20000, "progressFile": "stale.json"}],
                "summary": {"workers": 1, "totalUids": 1},
            },
        )
        ...
        self.assertEqual(
            ...
            [
                {
                    "key": "workers",
                    "python": [{"start": 1, "end": 20000, "progressFile": "batch-uid-progress-1-20000.json"}],
                    "js": [{"start": 1, "end": 20000, "progressFile": "stale.json"}],
                },
                {"key": "summary", "python": {"workers": 1, "totalUids": 20000}, "js": {"workers": 1, "totalUids": 1}},
            ],
        )
    ...
    def test_range_scraper_launcher_planner_builds_powershell_contract_without_filesystem(self):
        result = RangeScraperLauncherPlanner().build_plan(data_dir="server/data")
    ...
    def test_range_scraper_launcher_payload_runner_lives_with_scraper_logic(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            result = RangeScraperLauncherPayloadPlanRunner(data_dir).run()
    ...
    def test_range_scraper_launcher_payload_comparator_owns_progress_mismatch_contract(self):
        result = RangeScraperLauncherPayloadComparator().compare(
            {
                "workers": [{"start": 1, "end": 20000, "progressFile": "uid-range-progress-1-20000.json"}],
                "summary": {"workers": 1, "totalUids": 20000, "launchDelaySeconds": 3},
            },
            {
                "workers": [{"start": 1, "end": 20000, "progressFile": "stale.json"}],
        ...
        self.assertEqual(
            ...
            [
                {
                    "key": "workers",
                    "python": [{"start": 1, "end": 20000, "progressFile": "uid-range-progress-1-20000.json"}],
                    "js": [{"start": 1, "end": 20000, "progressFile": "stale.json"}],
                },
                {
                    "key": "summary",
                    "python": {"workers": 1, "totalUids": 20000, "launchDelaySeconds": 3},
    ...
    def test_fast_pipeline_launcher_plan_runner_matches_powershell_contract(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            result = FastPipelineLauncherPlanRunner(data_dir).run()
    ...
    def test_fast_pipeline_launcher_payload_runner_lives_with_scraper_logic(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            result = FastPipelineLauncherPayloadPlanRunner(data_dir).run()
    ...
    def test_fast_pipeline_launcher_comparator_reports_worker_mismatches(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            data_dir = root / "server" / "data"
            data_dir.mkdir(parents=True)
            ...
            result = FastPipelineLauncherContractComparator(data_dir, js_report_path).compare()
    ...
    def test_scraper_monitor_payload_runner_lives_with_scraper_monitor_logic(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            data_dir.mkdir()
            js_report_path = root / "js-monitor.json"
            (data_dir / "uid-discovery-progress.json").write_text(json.dumps({"stats": {"uidsAnalyzed": 1, "uidsFound": 3}}), encoding="utf-8")
            (data_dir / "uid-pipeline-1-1.json").write_text(json.dumps({"processed": {"1": "success"}, "stats": {"success": 1}}), encoding="utf-8")
            js_report_path.write_text(json.dumps({"combined": {"uidsAnalyzed": 1}, "pipeline": {"processed": 0}}), encoding="utf-8")

            result = ScraperMonitorPayloadRunner(data_dir, total_start=1, total_end=1, workers=1).run()
    ...
    def test_batch_bilibili_plan_comparator_uses_summary_contract_keys(self):
        self.assertFalse(hasattr(BatchBilibiliPlanContractComparator, "RESULT_KEYS"))
        self.assertEqual(
    ...
    def test_batch_uid_progress_comparator_uses_summary_contract_keys(self):
        self.assertFalse(hasattr(BatchUidProgressContractComparator, "RESULT_KEYS"))
        self.assertEqual(
            BatchUidProgressContractComparator(Path("progress.json"), Path("js.json")).summary.RESULT_KEYS,
            BatchUidProgressSummary.RESULT_KEYS,
        )
    ...
    def test_batch_uid_progress_payload_comparator_owns_result_key_mismatch_contract(self):
        result = BatchUidProgressPayloadComparator().compare(
            {
    ...
    def test_batch_uid_scrape_plan_comparator_uses_summary_contract_keys(self):
        self.assertFalse(hasattr(BatchUidScrapePlanContractComparator, "RESULT_KEYS"))
        self.assertEqual(
    ...
    def test_batch_uid_scrape_plan_payload_runner_lives_with_scraper_logic(self):
                ...
                json.dumps(
                    {
                        "progress": {
                            "scannedBvids": ["BV1", "BV2"],
                            "_uidComments": {"21": [{"message": "hit"}], "22": [{"message": ""}], "23": [{"message": "pending"}]},
                            "processedUids": {"21": "success"},
                            "stats": {"videosScanned": "2", "uidsFound": "3", "uidsAnalyzed": "1", "commentsCollected": "3"},
                        },
                        "database": {"users": {"21": {}, "99": {}}},
                    }
                ),
        ...
        self.assertTrue(result["ok"])
        self.assertEqual(result["discovery"]["scannedBvids"], 2)
        self.assertEqual(result["phase2"], {"processed": 1, "pending": 2, "skippableNoText": 1, "trainable": 1, "userDbUsers": 2})
        self.assertEqual(result["stats"]["commentsCollected"], 3)
        self.assertFalse(comparison["ok"])
    ...
    def test_batch_uid_range_plan_comparator_uses_summary_contract_keys(self):
        self.assertFalse(hasattr(BatchUidRangePlanContractComparator, "RESULT_KEYS"))
        self.assertEqual(
    ...
    def test_batch_uid_range_plan_payload_runner_lives_with_scraper_logic(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            payload_path.write_text(
                json.dumps(
                    {
                        "argv": ["--start=100", "--end=105", "--pages=6", "--phase2-only"],
                        "progress": {"scannedBvids": ["BV1"], "_uidComments": {"100": [{"message": "x"}], "106": [{"message": "y"}]}, "processedUids": {"100": "success"}},
                        "database": {"users": {"100": {}, "101": {}}},
                    }
                ),
                encoding="utf-8",
            )
            js_report_path.write_text(json.dumps({"input": {"start": 90}, "phase2": {"targetUids": 0}}), encoding="utf-8")

            result = BatchUidRangePayloadPlanRunner(payload_path).run()
            comparison = BatchUidRangePlanPayloadContractComparator(payload_path, js_report_path).compare()

        self.assertTrue(result["ok"])
        self.assertEqual(result["input"], {"start": 100, "end": 105, "pages": 6, "phase2Only": True})
        self.assertEqual(result["phase1"]["enabled"], False)
        self.assertEqual(result["phase2"], {"targetUids": 1, "processed": 1, "remaining": 0, "userDbUsers": 2})
        self.assertFalse(comparison["ok"])
    ...
    def test_uid_discovery_progress_payload_runner_lives_with_scraper_logic(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            data_dir = root / "server" / "data"
            data_dir.mkdir(parents=True)
            ...
            js_report_path.write_text(json.dumps({"analysis": {"processed": 0}}), encoding="utf-8")
        ...
        self.assertTrue(result["ok"])
        self.assertEqual(result["analysis"], {"processed": 1, "success": 1, "errors": 0, "skipped": 0, "remaining": 0})
        self.assertFalse(comparison["ok"])
    ...
    def test_uid_discovery_progress_comparator_uses_summary_contract_keys(self):
        self.assertFalse(hasattr(UidDiscoveryProgressContractComparator, "RESULT_KEYS"))
        self.assertEqual(
            UidDiscoveryProgressContractComparator(Path("data"), Path("js.json")).summary.RESULT_KEYS,
            UidDiscoveryProgressSummary.RESULT_KEYS,
        )
    ...
    def test_uid_discovery_plan_comparator_uses_summary_contract_keys(self):
        self.assertFalse(hasattr(UidDiscoveryPlanContractComparator, "RESULT_KEYS"))
        self.assertEqual(
    ...
    def test_uid_discovery_plan_payload_runner_lives_with_scraper_logic(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            payload_path.write_text(
                json.dumps(
                    {
                        "progress": {"phase": "analysis", "scannedBvids": ["BV1"], "processedUids": {"12": "success"}},
                        "comments": {"12": [{"message": "done"}], "30": [{"message": "pending"}]},
                        "database": {"users": {"12": {}, "30": {}}},
                    }
                ),
                encoding="utf-8",
            ...
            js_report_path.write_text(
                json.dumps(
                    {
                        "resume": {"phase": "analysis", "skipDiscovery": True, "scannedBvids": 1, "savedUidComments": 2},
                        "analysis": {"processed": 1, "pending": 1, "skippableNoText": 0, "trainable": 1, "userDbUsers": 2},
                    }
                ),
                encoding="utf-8",
            )
        ...
        self.assertTrue(result["ok"])
        self.assertEqual(result["resume"]["skipDiscovery"], True)
        self.assertEqual(result["analysis"]["trainable"], 1)
        self.assertTrue(comparison["ok"])
        self.assertEqual(comparison["js"]["resume"]["skipDiscovery"], True)
        self.assertEqual(comparison["python"]["analysis"]["trainable"], comparison["js"]["analysis"]["trainable"])
    ...
    def test_uid_range_progress_payload_runner_lives_with_scraper_logic(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            progress_path = root / "batch-uid-range-progress.json"
    ...
    def test_uid_range_progress_comparator_uses_summary_contract_keys(self):
        self.assertFalse(hasattr(UidRangeProgressContractComparator, "RESULT_KEYS"))
        self.assertEqual(
            UidRangeProgressContractComparator(Path("progress.json"), Path("js.json")).summary.RESULT_KEYS,
            UidRangeProgressSummary.RESULT_KEYS,
        )
    ...
    def test_near_target_override_terms_parser_lives_with_analysis_logic(self):
        ...
        terms = parser.parse(" alpha, beta;\ngamma| delta || ")

        self.assertEqual(terms, ["alpha", "beta", "gamma", "delta"])
    ...
    def test_video_keyword_discovery_report_request_owns_cli_dispatch(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            payload_path = root / "payload.json"
            js_report_path = root / "js-report.json"
            ...
            run_result = VideoKeywordDiscoveryReportRequest(payload_path).run()
            compare_result = VideoKeywordDiscoveryReportRequest(payload_path, compare_js_report_path=js_report_path).run()
    ...
    def test_harvest_options_request_compares_payload_contract(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            js_report_path.write_text(
                json.dumps({"mode": "coverage-runtime", "options": {"targetEvidence": 3, "maxActions": 5}}),
                encoding="utf-8",
            )

            result = HarvestOptionsRequest(payload_path=payload_path, compare_js_report_path=js_report_path).run()
    ...
    def test_coverage_audit_cli_runner_accepts_argv_contract(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            dictionary_path = root / "dictionary.json"
            js_audit_path = root / "js-audit.json"
            ...
            result = coverage_audit_cli.CoverageAuditRunner(["--dictionary", str(dictionary_path), "--js-audit", str(js_audit_path)]).run()

        self.assertTrue(result["ok"])
        self.assertEqual(result["mismatches"], [])
        self.assertEqual(result["python"]["coverage"]["weakTerms"], 1)

    def test_coverage_audit_request_runs_file_backed_contract(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            dictionary_path = root / "dictionary.json"
            js_audit_path = root / "js-audit.json"
            ...
            result = CoverageAuditRequest(dictionary_path=dictionary_path, js_audit_path=js_audit_path).run()
    ...
    def test_coverage_audit_dedicated_cli_runner_accepts_argv_contract(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            result = coverage_audit_cli.CoverageAuditCliRunner(["--dictionary", str(dictionary_path), "--js-audit", str(js_audit_path)]).run()
    ...
    def test_random_verification_json_result_bytes_lives_with_analysis_contract(self):
        ...
        encoded = random_verification_payload_json_result_bytes(payload)

        self.assertEqual(encoded, json_result_bytes(payload))
