...
export async function searchVideoKeywords(payload = {}, deps = {}) {
  ...
  if (favoriteRaw && videoLinks.length === 0) {
    const favId = (() => {
      const raw = String(favoriteRaw);
      const fidParam = raw.match(/[?&]fid=(\d+)/);
      if (fidParam) return fidParam[1];
      const medialistMatch = raw.match(/medialist[=/](\d+)/);
  ...
  const loadEvidenceSourceFallbackVideos = async () => {
    ...
    try {
      ...
      warnings.push(`existing evidence-source fallback: ${error.message}`);
      return [];
    }
  };
  if (videoLinks.length === 0) {
        ...
        if (searchGroup.length === 0 && allowFilteredDiscoveryFallback && !payload.abortSignal?.aborted) {
          for (const query of controversyQueries.slice(0, controversialPopularQueryLimit)) {
            try {
              ...
              warnings.push(`${query} (${controversialPopularSearchOrder || 'popular'}): ${error.message}`);
              if (payload.abortSignal?.aborted) break;
            }
          }
    ...
    if (
      ...
      try {
        ...
        warnings.push(`popular after search block: ${error.message}`);
      }
    }
    ...
    if (discoveredVideos.length === 0 && evidenceSourceVideoFallback && existingTermsOnly && targetExistingTerms.length > 0) {
      const fallbackVideos = await loadEvidenceSourceFallbackVideos();
      discoveredVideos = uniqueByKey(
        fallbackVideos.filter((video) => !excludeBvids.has(video.bvid)),
        (video) => video.bvid || video.sourceUrl || video.title,
      );
    ...
    if (discoveredVideos.length === 0) {
      ...
      return {
        ok: false,
        error: warnings[0] || 'No Bilibili videos were discovered from the backend discovery mode.',
        warnings: warnings,
        discoveredVideos,
        discoveryContextVideos,
        searchQueries,
