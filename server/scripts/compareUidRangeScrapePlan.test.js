...
test('uidRangeScrape accepts explicit python plan flag for dry-run planning', () => {
  const tempDir = mkdtempSync(join(tmpdir(), 'uid-range-scrape-explicit-python-plan-'));
  try {
    const payloadPath = join(tempDir, 'payload.json');
    const fakeModuleDir = join(tempDir, 'python_backend', 'cli');
    writeFileSync(
    ...
    writeFileSync(
      join(fakeModuleDir, 'uid_range_scrape_plan.py'),
      'print(\'{"ok":true,"fromExplicitPythonUidRangeScrapePlan":true,"resume":{"processed":12}}\')\n',
      'utf8',
    );

    const result = spawnSync('node', [resolve('server/scripts/uidRangeScrape.js'), '--plan-json', '--python-plan', '--payload', payloadPath], {
      cwd: tempDir,
      encoding: 'utf8',
    ...
    assert.equal(result.status, 0, result.stderr);
    const payload = JSON.parse(result.stdout);
    assert.equal(payload.fromExplicitPythonUidRangeScrapePlan, true);
    assert.equal(payload.resume.processed, 12);
  } finally {
    rmSync(tempDir, { recursive: true, force: true });
  }
});
