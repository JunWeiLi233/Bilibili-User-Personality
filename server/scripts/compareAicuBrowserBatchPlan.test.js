...
test('batchScrapeAicuBrowser accepts explicit python plan flag for dry-run planning', () => {
  const tempDir = mkdtempSync(join(tmpdir(), 'aicu-browser-python-plan-flag-'));
  try {
    const payloadPath = join(tempDir, 'payload.json');
    const fakeModuleDir = join(tempDir, 'python_backend', 'cli');
    writeFileSync(
      payloadPath,
      JSON.stringify({ argv: ['--start=100020', '--end=100022'], progress: {}, database: { users: {} } }, null, 2),
      'utf8',
    );
    mkdirSync(fakeModuleDir, { recursive: true });
    writeFileSync(join(tempDir, 'python_backend', '__init__.py'), '', 'utf8');
    ...
    writeFileSync(
      join(fakeModuleDir, 'aicu_browser_batch_plan.py'),
      'print(\'{"ok":true,"fromExplicitPythonAicuBrowserPlan":true,"range":{"effectiveStart":100020},"browser":{"command":"explicit-python-browser-plan"}}\')\n',
      'utf8',
    );

    const result = spawnSync(
      'node',
      [resolve('server/scripts/batchScrapeAicuBrowser.js'), '--plan-json', '--python-plan', '--payload', payloadPath],
      {
        cwd: tempDir,
        encoding: 'utf8',
        env: {
    ...
    assert.equal(result.status, 0, result.stderr);
    const payload = JSON.parse(result.stdout);
    assert.equal(payload.fromExplicitPythonAicuBrowserPlan, true);
    assert.equal(payload.browser.command, 'explicit-python-browser-plan');
  } finally {
    rmSync(tempDir, { recursive: true, force: true });
  }
});
