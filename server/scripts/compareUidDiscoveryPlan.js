...
async function runPythonPlanComparison({ payloadPath, jsReportPath }) {
  const { stdout } = await execFileAsync(
    'python',
    ['-m', 'python_backend.cli.uid_discovery_plan', '--payload', payloadPath, '--compare-js-report', jsReportPath],
    {
      cwd: process.cwd(),
      env: { ...process.env, PYTHONUTF8: '1', PYTHONIOENCODING: 'utf-8' },
      maxBuffer: 10 * 1024 * 1024,
...
export async function compareUidDiscoveryPlanSuite({
  ...
  for (const fixture of fixtures) {
    results.push(await compareUidDiscoveryPlan({ fixture, runJs, runPython, runCompare }));
  }
  return {
