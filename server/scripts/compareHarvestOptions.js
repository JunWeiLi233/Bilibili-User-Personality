...
async function runPythonOptionsComparison({ payloadPath, jsReportPath }) {
  const { stdout } = await execFileAsync(
    'python',
    ['-m', 'python_backend.cli.harvest_options', '--payload', payloadPath, '--compare-js-report', jsReportPath],
    {
      cwd: process.cwd(),
      env: { ...process.env, PYTHONUTF8: '1', PYTHONIOENCODING: 'utf-8' },
      maxBuffer: 10 * 1024 * 1024,
...
async function compareHarvestOptionsSingle({
  payload,
  fixture,
  runJs = runJsOptions,
...
export async function compareHarvestOptions({
  ...
  if (fixtureNames) {
    ...
    for (const name of fixtureNames.length ? fixtureNames : DEFAULT_FIXTURE_NAMES) {
      results.push(await compareHarvestOptionsSingle({ fixture: name, runJs, runPython, runCompare }));
    }
  ...
  return compareHarvestOptionsSingle({ payload: payload || DEFAULT_PAYLOAD, fixture, runJs, runPython, runCompare });
}
