...
export function compareCoverageHarvestLoopPlanObjects(pythonResult = {}, jsResult = {}) {
...
async function runJsPlan({ payloadPath }) {
  const { stdout } = await execFileAsync('node', ['server/scripts/runCoverageHarvestLoop.js', '--plan-json', '--payload', payloadPath], {
    cwd: process.cwd(),
