...
export const FAST_PIPELINE_LAUNCHER_PLAN_FIXTURES = {
  'default-data-dir': DEFAULT_PAYLOAD,
  'custom-data-dir': {
    dataDir: join(tmpdir(), 'fast-pipeline-launcher-custom-data'),
  },
};
...
function resolvePayload({ fixture = 'default-data-dir', payload } = {}) {
  ...
  return { name, payload: FAST_PIPELINE_LAUNCHER_PLAN_FIXTURES[name] || DEFAULT_PAYLOAD };
}
...
export async function compareFastPipelineLauncherPlan({
  ...
  if (fixtureNames) return compareFastPipelineLauncherPlanSuite({ fixtures: fixtureNames, runJs, runPython });
...
export async function compareFastPipelineLauncherPlanSuite({
  ...
  for (const fixture of fixtures) {
    results.push(await compareFastPipelineLauncherPlan({ fixture, runJs, runPython }));
  }
  return {
...
async function main() {
  const result = await compareFastPipelineLauncherPlanSuite();
