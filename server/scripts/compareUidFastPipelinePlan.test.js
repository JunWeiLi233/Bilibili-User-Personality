...
test('uidPipelineFast accepts explicit python plan flag for dry-run planning', () => {
  const tempDir = mkdtempSync(join(tmpdir(), 'uid-fast-pipeline-explicit-python-plan-'));
  try {
    const payloadPath = join(tempDir, 'payload.json');
    const fakeModuleDir = join(tempDir, 'python_backend', 'cli');
    writeFileSync(
      payloadPath,
      JSON.stringify({ argv: ['--start=2', '--end=4'], progress: { processed: {}, stats: {} }, database: { users: {} } }, null, 2),
      'utf8',
    );
    mkdirSync(fakeModuleDir, { recursive: true });
    writeFileSync(join(tempDir, 'python_backend', '__init__.py'), '', 'utf8');
    ...
    writeFileSync(
      join(fakeModuleDir, 'uid_fast_pipeline_plan.py'),
      'print(\'{"ok":true,"fromExplicitPythonUidFastPipelinePlan":true,"network":{"mode":"explicit-python-fast-plan"}}\')\n',
      'utf8',
    );

    const result = spawnSync('node', [resolve('server/scripts/uidPipelineFast.js'), '--plan-json', '--python-plan', `--payload=${payloadPath}`], {
      cwd: tempDir,
      encoding: 'utf8',
    ...
    assert.equal(result.status, 0, result.stderr);
    const payload = JSON.parse(result.stdout);
    assert.equal(payload.fromExplicitPythonUidFastPipelinePlan, true);
    assert.equal(payload.network.mode, 'explicit-python-fast-plan');
  } finally {
    rmSync(tempDir, { recursive: true, force: true });
  }
});
