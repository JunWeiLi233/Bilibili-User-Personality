...
const RESULT_KEYS = ['input', 'range', 'resume', 'database', 'limits', 'pacing', 'retry', 'browser', 'sampleRequests', 'progress'];
...
export const DEFAULT_PAYLOAD = {
  ...
  progress: {
    lastUid: 100002,
    completed: 2,
    errors: [{ uid: '100001', error: 'HTTP 412' }],
  },
  database: {
    users: {
      99999: { uid: '99999' },
...
export function compareBatchBilibiliPlanObjects(pythonResult = {}, jsResult = {}) {
...
async function runJsPlan({ payloadPath }) {
  const { stdout } = await execFileAsync('node', ['server/scripts/batchScrapeBilibili.js', '--plan-json', '--payload', payloadPath], {
    cwd: process.cwd(),
...
async function runPythonPlanComparison({ payloadPath, jsReportPath }) {
  const { stdout } = await execFileAsync(
    'python',
    ['-m', 'python_backend.cli.batch_bilibili_plan', '--payload', payloadPath, '--compare-js-report', jsReportPath],
    {
      cwd: process.cwd(),
      env: { ...process.env, PYTHONUTF8: '1', PYTHONIOENCODING: 'utf-8' },
      maxBuffer: 10 * 1024 * 1024,
...
export async function compareBatchBilibiliPlan({
  ...
  if (fixtureNames) return compareBatchBilibiliPlanSuite({ fixtures: fixtureNames, runJs, runPython, runCompare });
...
export async function compareBatchBilibiliPlanSuite({
  ...
  for (const fixture of fixtures) {
    results.push(await compareBatchBilibiliPlan({ fixture, runJs, runPython, runCompare }));
  }
  return {
