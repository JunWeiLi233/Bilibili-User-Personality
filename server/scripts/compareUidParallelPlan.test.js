...
test('uidParallelAnalyzer accepts explicit python plan flag for dry-run planning', () => {
  const tempDir = mkdtempSync(join(tmpdir(), 'uid-parallel-explicit-python-plan-'));
  try {
    const payloadPath = join(tempDir, 'payload.json');
    const fakeModuleDir = join(tempDir, 'python_backend', 'cli');
    writeFileSync(
      payloadPath,
      JSON.stringify({ argv: ['--worker=1', '--workers=3'], comments: { 102: [{ message: 'b' }] }, progress: { processed: {}, stats: {} }, database: { users: {} } }, null, 2),
      'utf8',
    );
    mkdirSync(fakeModuleDir, { recursive: true });
    writeFileSync(join(tempDir, 'python_backend', '__init__.py'), '', 'utf8');
    ...
    writeFileSync(
      join(fakeModuleDir, 'uid_parallel_plan.py'),
      'print(\'{"ok":true,"fromExplicitPythonUidParallelPlan":true,"assignment":{"pending":3}}\')\n',
      'utf8',
    );

    const result = spawnSync('node', [resolve('server/scripts/uidParallelAnalyzer.js'), '--plan-json', '--python-plan', `--payload=${payloadPath}`], {
      cwd: tempDir,
      encoding: 'utf8',
    ...
    assert.equal(result.status, 0, result.stderr);
    const payload = JSON.parse(result.stdout);
    assert.equal(payload.fromExplicitPythonUidParallelPlan, true);
    assert.equal(payload.assignment.pending, 3);
  } finally {
    rmSync(tempDir, { recursive: true, force: true });
  }
});
